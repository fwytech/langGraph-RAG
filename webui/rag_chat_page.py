...
# 定义函数用于获取RAG聊天响应
def get_rag_chat_response(platform, model, temperature, input, selected_tools, KBS, tool_name_map):
    # 获取RAG图
    app = get_rag_graph(platform, model, temperature, selected_tools, KBS)
    # 返回图的响应
    return graph_response(graph=app, input=input, tool_name_map=tool_name_map)
...
def rag_chat_page():
    st.markdown(
        ...
        "<h2 style='margin:0;text-align:center;line-height:1.6;'>基于 LangChain 1.x+  LangGraph 1.x+RAG 技术构建金融智能客服应用</h2>"
        "</div>",
